...
export const AdminUsersTab: React.FC = () => {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (r.ok) {
      setCreateStatus(`User "${createName}" created successfully.`);
      setCreateName('');
      setCreateEmail('');
      setCreatePassword('');
      setCreateRole('user');
      ...
      const body = await r.json().catch(() => ({}));
      setCreateStatus(`Error: ${body.error || 'Failed to create user'}`);
    }
    setIsCreating(false);
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        <h3 className="font-semibold text-slate-800 mb-4">Invite User via Email</h3>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6">
        ...
        >
          <h3 className="font-semibold text-slate-800">Create User Directly</h3>
          <span className="text-slate-400 text-sm">{showCreateForm ? '▲ Hide' : '▼ Show'}</span>
        </button>
        ...
        {showCreateForm && (
          <form onSubmit={handleCreateUser} className="mt-4 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  required
                  disabled={isCreating}
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 disabled:bg-slate-100 text-sm"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <select
                  ...
                  disabled={isCreating}
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 disabled:bg-slate-100 text-sm bg-white"
                >
            ...
            <button
              type="submit"
              disabled={isCreating || !createName || !createEmail || !createPassword}
              className="px-5 py-2 bg-emerald-600 text-white font-semibold rounded-lg hover:bg-emerald-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-sm"
            >
              {isCreating ? 'Creating...' : 'Create User'}
            </button>
            {createStatus && (
              <p className={`text-sm ${createStatus.startsWith('Error') ? 'text-red-600' : 'text-green-600'}`}>
                {createStatus}
              </p>
            )}
          </form>
